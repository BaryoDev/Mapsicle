...
namespace Mapsicle.SourceGen
{
    ...
    public sealed class MapperGenerator : IIncrementalGenerator
    {
        ...
        private static void Preamble(StringBuilder sb)
        {
            ...
            sb.AppendLine();
            sb.AppendLine("// A member marked obsolete as a warning is still mapped, because the runtime engine maps it");
            sb.AppendLine("// and the two lanes have to agree, so the warning is suppressed here rather than handed to a");
            sb.AppendLine("// consumer who cannot edit this file. Obsolete as an ERROR is not listed, and cannot be: a");
            sb.AppendLine("// pragma disables warnings and CS0619 is an error. Those pairs are refused instead.");
