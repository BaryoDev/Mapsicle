...
namespace Mapsicle.SourceGen
{
    ...
    public sealed class MapperGenerator : IIncrementalGenerator
    {
        ...
        ///
        /// It lives in the global namespace so it is in scope wherever the source type is used. An
        /// extension is only a candidate if its containing namespace is imported, and a generated
        /// one nobody imports would silently never bind, which is the worst outcome available here:
        /// no error, no speedup, and a benchmark that says it worked.
        /// </remarks>
        private static string EmitExtensions(List<MapPlan> plans)
        {
            ...
            sb.AppendLine();
            sb.AppendLine("/// <summary>Compile-time bound MapTo for the pairs this assembly declared.</summary>");
            sb.AppendLine("internal static class MapsicleGeneratedExtensions");
            sb.AppendLine("{");

            foreach (var group in bySource)
            {
                var src = group.Key;
                sb.AppendLine($"    /// <summary>Maps a <see cref=\"{Escape(src)}\"/> into <typeparamref name=\"TDest\"/>.</summary>");
                sb.AppendLine($"    public static TDest? MapTo<TDest>(this {src}? source)");
                sb.AppendLine("    {");
                ...
                foreach (var plan in group)
                {
                    ...
                    sb.AppendLine($"        if (typeof(TDest) == typeof({dst}))");
                    sb.AppendLine("        {");
                    sb.AppendLine($"            return (TDest)(object)new {dst}");
                    sb.AppendLine("            {");
                ...
                sb.AppendLine("        // Not a pair this assembly declared, so the engine resolves it as it always has.");
                sb.AppendLine("        return global::Mapsicle.Mapper.MapTo<TDest>((object)source);");
                sb.AppendLine("    }");
                sb.AppendLine();
            }
